...
interface PetContractPageProps {
  insurer: string;
  amountLabel: string;
  onBack: () => void;
...
const contractNumber = '6620488134';
const paymentUrl = 'https://platebnibrana.example/pay/6620488134';
...
export function PetContractPage({
  insurer,
  amountLabel,
  onBack,
  onDownloadRecord,
}: PetContractPageProps) {
  const router = useRouter();

  const documents: DocItem[] = [
    { name: 'Pojistná smlouva', desc: 'Smlouva o pojištění mazlíčka', source: 'Pojišťovna' },
    {
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-success/30 bg-success-bg px-6 py-5">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted">
            Pojištění mazlíčka u {insurer} je připravené. Pro aktivaci uhraďte pojistné.
          </p>
